...
public sealed class RemoveGameStateSystem : ICleanupSystem {

    readonly IGroup<StateEntity> _group;
    readonly List<StateEntity> _buffer = new List<StateEntity>();

    public RemoveGameStateSystem(Contexts contexts) {
        _group = contexts.state.GetGroup(StateMatcher.Game);
    }

    public void Cleanup() {
        foreach (var e in _group.GetEntities(_buffer)) {
            e.transitionGame = false;
        }
    }
}
